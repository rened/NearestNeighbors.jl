...
function knn_kernel!{T}(tree::KDTree{T},
    ...
    if isleaf(tree.tree_data.n_internal_nodes, index)
        add_points_knn!(best_dists, best_idxs, tree, index, point, false, timeout)
        return
    end
    ...
    knn_kernel!(tree, close, point, best_idxs, best_dists, min_dist, timeout)  # hotspot
    ...
    if new_min < best_dists[1]
        knn_kernel!(tree, far, point, best_idxs, best_dists, new_min, timeout)  # hotspot
    end
    return
end
